...
func TestReadWriteCornerCases(t *testing.T) {
	...
	arenaSize := 8 * 1024
	bq, err := NewBigQueue(testDir, SetArenaSize(arenaSize), SetMemorySize(8*1024*10))
	if err != nil {
		t.Fatalf("unable to get BigQueue: %v", err)
	}
	...
	for i := 1; i < 10; i++ {
		...
		bq.Close()
		if bqTemp, err := NewBigQueue(testDir, SetArenaSize(arenaSize), SetMemorySize(8*1024*10)); err != nil {
			t.Fatalf("unable to get BigQueue: %v", err)
		} else {
			bq = bqTemp
		}
